...
window.onload = () => {
  let areaUrl = `https://apis.data.go.kr/B551011/KorService1/areaCode1?serviceKey=${serviceKey}&numOfRows=50&pageNo=1&MobileOS=ETC&MobileApp=AppTest&_type=json`;
  fetch(areaUrl)
...
function changeOp1() {
  ...
  let areaDetailUrl = `https://apis.data.go.kr/B551011/KorService1/areaCode1?serviceKey=${serviceKey}&numOfRows=30&pageNo=1&MobileOS=ETC&MobileApp=AppTest&areaCode=${selectEl.value}&_type=json`;
  fetch(areaDetailUrl)
    .then((response) => response.json())
    .then((data) => {
      for (let item of data.response.body.items.item) {
...
function createMarker(contentId, contentTypeId, title, lat, lng) {
  ...
  var imageSrc = `./asset/img/marker/${markerImgFile[contentTypeId]}`, // 마커이미지의 주소입니다
